...
class Graph {
    ...
    addEdge(u, v, w) {
        // Find if edge already exists, update if it does
    ...
    shortestPath(src, dest, emergency, trafficLevel) {
        ...
        // Simple priority queue using array and sort
        ...
        while (pq.length > 0) {
            ...
            for (let neighbor of neighbors) {
                ...
                if (weight === INT_MAX) continue; // Blocked road
...
let graph;
let nodesData = new Map(); // Store node positions: {id: {x, y}}
let edgesData = []; // Store edge visual info: {u, v, w, elementGroup}
let nextNodeId = 6;
...
function initGraph() {
    ...
    // Predefined positions for default nodes (for a nice layout)
...
// Create Edge visually and logically
function addEdgeAction(u, v, w) {
    ...
    // Check if edge visually exists to update or create
